...
def preflight(args: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "policy_n_action_steps": POLICY_ACTION_STEPS,
...
def run_hardware_trial(args: argparse.Namespace, preflight_record: dict) -> None:
    ...
    model.to("cuda")
    model.eval()
    if model.config.chunk_size != POLICY_CHUNK_SIZE:
        raise RuntimeError("Loaded checkpoint SmolVLA chunk size changed.")
    if model.config.n_action_steps != POLICY_ACTION_STEPS:
        raise RuntimeError("Loaded checkpoint SmolVLA n_action_steps changed.")
    ...
    try:
        ...
        def policy_tick(
            ...
            policy_input = prepare_observation_for_inference(
                ...
                task='Task 1 PickLift v1: grasp the 2 cm red cube and lift it >=5 cm with bilateral finger hold',
